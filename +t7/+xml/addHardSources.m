...
for ss = 1:length(sim.Grid.HardSources)
    ...
    if length(src.timeData) ~= 0
        %fname = t7.xml.randomName('__hardsource_time_', '', 8);
    ...
    if length(src.maskData) ~= 0
        %fname = t7.xml.randomName('__hardsource_mask_', '', 8);
        ...
        fclose(fh);
        t7.xml.writeSourceSpec(sim, src, 'AutoMaskFile', fname);
    end
    
    if length(src.spaceTimeData) ~= 0
        %fname = t7.xml.randomName('__hardsource_spacetime_', '', 8);
